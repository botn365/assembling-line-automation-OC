...
while true do
  ...
  if matchnumber then
    local amount = findmatch.getMax(recipymap,matchnumber,readchest.simpleinventory,fluid),readchest.simpleinventory,fluid) --check how many recipys it can make
    if amount > 14 then 
      amount = 14
    end
    if amount > 0 then
      local loadmap = findmatch.makeLoadMap(recipymap,matchnumber,amount)-- makes the load map of the recipy
